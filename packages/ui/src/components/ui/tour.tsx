...
export interface TourStep {
  ...
  onNext?: () => void | Promise<void>; // Added onNext callback
}
...
export function TourProvider({
  ...
  // Updated nextStep function to handle onNext callback
  const nextStep = useCallback(async () => {
    // If current step has onNext callback, execute it first
    if (currentStep >= 0 && currentStep < steps.length && steps[currentStep]?.onNext) {
      try {
        await steps[currentStep].onNext?.();
      } catch (error) {
        console.error('Error in tour onNext callback:', error);
      }
    }
