...
export default function WaitlistForm() {
  ...
  if (submitted) {
    return (
      ...
      >
        ...
        <h3 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white">You&apos;re on the list!</h3>
        <p className="text-gray-600 dark:text-gray-300">We&apos;ll notify you as soon as we launch.</p>
      </motion.div>
    );
  }
  ...
  return (
    ...
    >
      <Card className="backdrop-blur-sm bg-white/80 dark:bg-gray-900/80 border-0 relative overflow-hidden shadow-lg dark:shadow-gray-900/20">
        ...
        <CardContent className="p-8">
          <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
            <div className="space-y-2">
              <Label htmlFor="email" className="flex items-center gap-2 text-sm font-medium text-gray-900 dark:text-white">
              ...
              <Input
                ...
                required
                className={`h-12 bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm border-gray-200 dark:border-gray-700 focus:border-[#4361ee] focus:ring-[#4361ee] text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 ${
            ...
            <div className="space-y-3">
              ...
              >
                <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                  <Label
                    htmlFor="brand"
                    className={`flex items-center justify-center gap-2 p-4 rounded-lg border-2 cursor-pointer transition-all text-gray-900 dark:text-white ${
                      formData.userType === 'BRAND'
                        ? 'border-[#4361ee] bg-[#4361ee]/5 dark:bg-[#4361ee]/10'
                        : 'border-gray-200 dark:border-gray-700 hover:border-[#4361ee]/50 bg-white/50 dark:bg-gray-800/50'
                    }`}
                  >
              ...
              {errors.userType && (
                <p className="text-sm text-red-500 dark:text-red-400 mt-1">{errors.userType}</p>
              )}
            </div>

            <div className="space-y-2">
              <Label htmlFor="intention" className="flex items-center gap-2 text-sm font-medium text-gray-900 dark:text-white">
                Goals & Expectations
              </Label>
              <Textarea
                id="intention"
                ...
                required
                className={`min-h-[100px] bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm border-gray-200 dark:border-gray-700 focus:border-[#4361ee] focus:ring-[#4361ee] text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 ${
                  errors.intention ? 'border-red-500 dark:border-red-400' : ''
                }`}
              />
              {errors.intention && (
                <p className="text-sm text-red-500 dark:text-red-400 mt-1">{errors.intention}</p>
              )}
            </div>
            ...
            <Button
              type="submit"
              disabled={isLoading}
              className="w-full h-12 bg-[#4361ee] hover:bg-[#4361ee]/90 text-white transition-all shadow-lg hover:shadow-xl"
            >
              {isLoading ? (
                <>
