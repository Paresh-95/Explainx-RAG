...
export default function SummaryQuizGeneratorClient() {
  ...
  const handleGenerateQuiz = async (e: React.FormEvent) => {
    ...
    setQuiz([]);
    try {
      const res = await fetch("/api/tools/summary-quiz-generator", {
        method: "POST",
      ...
      if (!Array.isArray(data.quiz) || data.quiz.length === 0 || !data.quiz[0].question) {
        throw new Error("Quiz generation failed. Please try again with a different summary.");
      }
      setQuiz(data.quiz);
      setShowQuiz(true);
    } catch (err: any) {
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center bg-background text-foreground transition-colors duration-500 pt-24 md:pt-28 pb-12">
      ...
      <div className="w-full max-w-5xl px-4 py-8 bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-zinc-700 flex flex-col items-center animate-fade-in">
        <h1 className="text-3xl md:text-4xl font-bold heading text-center mb-6 text-gray-900 dark:text-gray-100">Summary to Quiz Generator</h1>
        {/* Settings Panel */}
        {!showQuiz && (
        ...
        {!showQuiz ? (
          ...
          >
            ...
            <textarea
              ...
              placeholder="Paste your summary, notes, or condensed material here..."
              value={inputText}
          ...
          <div className="w-full flex flex-col md:flex-row gap-6 animate-slide-in mt-4">
            ...
            <div className="w-full bg-white dark:bg-zinc-900 rounded-xl shadow p-6 border border-gray-200 dark:border-zinc-700 min-h-[400px] flex flex-col relative">
              ...
              <div className="mb-6">
                <label className="block font-semibold text-lg mb-2 text-gray-900 dark:text-gray-100">Provided Summary Preview</label>
