...
export async function POST(req: Request) {
  try {
    ...
    // Notify on Discord about the subscription
    try {
      const discordMessage = isExistingSubscriber
        ? `Existing subscriber updated: ${validatedData.email}`
        : `New subscriber added: ${validatedData.email}`;
