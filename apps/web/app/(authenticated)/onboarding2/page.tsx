...
export default function OnboardingPage() {
  ...
  const handleNext = async () => {
    ...
    if (currentStep < 4) {
      ...
      // Redirect after 1 second (short delay so toast is visible briefly)
      setTimeout(() => {
        router.push('/dashboard');
      }, 1000); // 1000ms is enough to show the toast briefly
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 h-full min-h-screen">
        ...
        <div className="md:col-span-7 col-span-1 h-full w-full p-4 md:p-8 flex flex-col">
          ...
          <div className="mt-auto pt-8 flex flex-col md:flex-row justify-between gap-4 md:gap-0">
            ...
            <button
              onClick={handleNext}
              className={`px-6 py-2 rounded-md bg-purple-600 text-white font-medium transition-all ${(currentStep === 1 && !selectedOption) ||
